...
def build3DMesh(grid2D, ncDS) :
    ...
    nczShift = ncDS['zShift'].values
    nc_dz = ncDS['dx'].values
    nx = 68
    ny = 64
    nk = 16
...
def buildQuadMesh(ds) :
    ll = (ds['Rxy_lower_left_corners'].values, ds['Zxy_lower_left_corners'].values)
    lr = (ds['Rxy_lower_right_corners'].values, ds['Zxy_lower_right_corners'].values)
    ur = (ds['Rxy_upper_right_corners'].values, ds['Zxy_upper_right_corners'].values)
    ul = (ds['Rxy_upper_left_corners'].values, ds['Zxy_upper_left_corners'].values)
    phi = ds.values #ds['phi'].values
    zShift = ds['zShift'].values
    ...
    for i in i_range :
        for j in j_range :
            ...
            phiVar.InsertNextValue(phi[i,j,0])
            zShiftVar.InsertNextValue(zShift[i,j])
...
fname = './xpt-netcdf/BOUT.dmp.*.nc'

# Open the NetCDF file
_grid = xr.open_dataset('./1_5_torus/65402_68x32_revIp_wide_fixBp_curv.nc')
f = ad.FileReader('./1_5_torus/BOUT.dmp.bp')
zShift = f.read('zShift')

ds = xb.open_boutdataset('./xpt-netcdf/BOUT.dmp.*.nc', geometry="toroidal", gridfilepath="g185472-68x64-curv.nc")

ds = ds.isel(t=0)
print('nz= ', ds['dz'].values)
